...
Lưu Ý! Đây Là Sản Phẩm Được DongDev Duy Trì và Phát Triển ( Fca Được Mod Lại Từ Scr Của Fca-Rqzax, Src gốc Fca-Horizon-Remake ), Nếu Có Lỗi Hãy Thử Sử Dụng Sang Sản Phẩm Khác !
...
```bash
npm i fca-project-dongdev
```
or
```bash
npm install fca-project-dongdev
```
...
```javascript
const login = require("fca-project-dongdev"); // lấy từ lib ra 
...
```js
    var login = require('fca-project-dongdev')
```
...
```js
const fs = require("fs");
const login = require("fca-project-dongdev");
