...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/dongdev06/Fca-Project-Dongdev/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
          ...
          log.warn("[ FCA-PMD ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/fca-project-dongdev/package.json')).version,JSON.parse(res.body.toString()).version));
                    if (globalThis.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-PMD ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install fca-project-dongdev@latest', { stdio: 'inherit' });
                                ...
                                try {
                                    require.resolve('dongdev-sp');
                                }
                                catch (e) {
                                    logger.Normal(Language.InstallSupportTool);
                                        execSync('npm install dongdev-sp@latest', { stdio: 'inherit' });
                                    process.exit(1);
                                }
                                    var fcasp = require('dongdev-sp');
                                try {
                                    fcasp.onError()
                                }
                                catch (e) {
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-project-dongdev/package.json')).version;
    console.clear();
